...
* Use existing files for example php repository and class code
...
Here’s an expanded and polished version of your **Parts (Physical)** section, integrating your notes and adding clarity to the functional relationships and marble size logic:
...
Moments are usually grouped into **Snippets**, which are self-contained narrative or mechanical sequences. For example, a snippet might show “G Choppy cutting the Triple Splitter” from frames 1080 to 1308. This format allows the animation to be browsed by time, by part, or by character, providing multiple ways to experience the build.
...
* I want to add Moments to Parts pages.  This should be possible in at least two ways:

1. Mimic the Moment Create page (on the Part edit page): Have empty fields for Take ID (dropdown from TABLE takes), frame_start (int), frame_end (int), and notes (what happened).  Click save and it's added via AJAX to TABLE Moments and then connected to the Part via parts_2_moments table.
2. Filter on a dropdown of existing moments from TABLE moments, especially if the Parts alias is in the moment.notes field

* I want to add Workers to Moments.  workers_2_moments because there can be more than one worker per Moment.  "Mr Greene give 19possd to Reversible Guy"

* I want the frontend to show the "History" of each Part or Worker based on the parts_2_moments and workers_2_moments.   I guess Moments should have dates, because ultimately frames have dates, but those are not recorded yet except in some Phrases which included dates, but not all Moments have Phrases, and not all Phrases have accurate dates.


### Future steps

* Somehow figure out how to store Frames in the database.  They are currently in Dragonframe 4 application on my local computer, but these are not yet available on the website.

* Dragonframe has an XML file to describe which frames are what; maybe we should write a parser for each Take's XML file and then send the correct frames to S3 and log them in the DB.

* Once the frames are stored reliably available to the website, I'd like to create video snippets based on Moments, given that each Moment has frame start and end.

* Make something longer than a Moment, perhaps a Sequence (of Moments) which can show all the related Frames as a video.
